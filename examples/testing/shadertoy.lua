    ...
    items = {
        ui.shader({
            ...
            fragment = [[
                ...
                // Shadertoy compatibility - map to our uniforms
                #define iTime u_time
                #define iResolution vec2(600.0, 400.0)
                ...
                void main() {
                    ...
                    // Note: Original uses iChannel0 texture, we'll simulate with noise
                    vec3 texture_sim = vec3(noise(uv * 0.6 + vec2(.5, .1)) * 0.5 + 0.5);
                    color += (texture_sim * 0.01 * pow((r.y + r.y) * .65, 5.0) + 0.055) * mix(vec3(.9, .4, .3), vec3(.7, .5, .2), uv.y);
