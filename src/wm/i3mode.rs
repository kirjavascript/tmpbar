...
pub fn listen(signal: Signal<String>) -> Result<(), I3IpcError> {
    ...
    subscribe_to_events(&mut stream)?;
        ...
        loop {
            ...
            if event_type & 0x7FFFFFFF == 2 {
                ...
                if let Some(next_mode) = extract_change(&payload) {
                    signal.send(next_mode.to_string());
                }
            }
        }
    });
...
fn subscribe_to_events(stream: &mut UnixStream) -> Result<(), I3IpcError> {
    send_message(stream, 2, "[\"mode\"]".as_bytes())?;

    let (_type, payload) = receive_message(stream)?;
...
fn send_message(stream: &mut UnixStream, message_type: u32, payload: &[u8]) -> Result<(), I3IpcError> {
...
fn receive_message(stream: &mut UnixStream) -> Result<(u32, Vec<u8>), I3IpcError> {
    let mut header = [0; 14];
